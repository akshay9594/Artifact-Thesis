...
int main(int argc, char * argv[])
{
	...
	{
		int * reduce0 = (int * )malloc(((75000*(13+1))*(13+1))*sizeof (int));
		int reduce_span_0;
		for (reduce_span_0=0; reduce_span_0<((75000*(13+1))*(13+1)); reduce_span_0 ++ )
		{
		...
		#pragma omp critical
		{
			for (reduce_span_0=0; reduce_span_0<((75000*(13+1))*(13+1)); reduce_span_0 ++ )
			{
				colidx[reduce_span_0]+=reduce0[reduce_span_0];
			}
		}
	}
	/* --------------------------------------------------------------------- */
	...
	print_results("CG", Class, 75000, 0, 0, 75, t, mflops, "          floating point", verified, "3.3.1", "20 Jul 2023", "gcc", "$(CC)", "-lm", "-I../common", "-g -Wall -O3 -fopenmp -mcmodel=large", "-g -O3 -fopenmp -mcmodel=large", "randdp");
	/* --------------------------------------------------------------------- */
	/* More timers */
	/* --------------------------------------------------------------------- */
	if (timeron)
...
static void conj_grad(int colidx[], int rowstr[], double x[], double z[], double a[], double p[], double q[], double r[], double * rnorm)
{
	...
	/* ----> */
	/* --------------------------------------------------------------------- */
	#pragma cetus private(alpha, beta, cgit, d, j, k, rho0, sum) 
	...
	for (cgit=1; cgit<=cgitmax; cgit ++ )
	{
		...
		*/
		...
		#pragma cetus parallel 
		#pragma omp parallel for if((10000<((5L+(-4L*firstcol))+(4L*lastcol)))) private(j)
		for (j=0; j<((lastcol-firstcol)+1); j ++ )
		{
			z[j]=(z[j]+(alpha*p[j]));
			r[j]=(r[j]-(alpha*q[j]));
	...
	#pragma cetus reduction(+: sum) 
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<((5L+(-4L*firstcol))+(4L*lastcol)))) private(d, j) reduction(+: sum)
	for (j=0; j<((lastcol-firstcol)+1); j ++ )
	{
		d=(x[j]-r[j]);
		sum=(sum+(d*d));
...
static void sparse(double a[], int colidx[], int rowstr[], int n, int nz, int nozer, int arow[], int acol[][(13+1)], double aelt[][(13+1)], int firstrow, int lastrow, int nzloc[], double rcond, double shift)
{
	...
	#pragma cetus parallel 
	#pragma omp parallel for if((10000<((7L+(-3L*firstrow))+(3L*lastrow)))) private(j)
	for (j=0; j<(nrows+1); j ++ )
	{
		rowstr[j]=0;
	}
