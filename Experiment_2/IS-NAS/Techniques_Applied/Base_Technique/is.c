...
double randlc(double * X, double * A)
{
	...
	if (KS==0)
	{
		...
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i) reduction(: R23, T23)
		*/
		for (i=1; i<=23; i ++ )
		{
			R23=(0.5*R23);
		...
		#pragma cetus parallel 
		/*
		Disabled due to low profitability: #pragma omp parallel for private(i) reduction(: R46, T46)
		*/
		for (i=1; i<=46; i ++ )
		{
			R46=(0.5*R46);
...
int main(int argc, char * * argv)
{
	...
	*/
	...
	#pragma cetus private(iteration) 
