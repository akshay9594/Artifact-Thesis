...
int hypre_CSRMatrixMatvec(double alpha, hypre_CSRMatrix * A, hypre_Vector * x, double beta, hypre_Vector * y, double time_array[], int id)
{
	...
	*/
	...
	if ( ! (num_vectors==y->num_vectors))
	{
		...
		hypre_error_handler("csr_matvec.c", 96, 1);
	}
	;
	if (num_cols!=x_size)
	{
	...
	*/
	...
	if (num_rownnz<(xpar*num_rows))
	{
		...
		#pragma cetus private(i, jj, tempx) 
		...
		for (i=0; i<num_rownnz; i ++ )
		{
			...
			*/
			if (num_vectors==1)
			{
				...
				#pragma cetus parallel 
				#pragma omp parallel for if((10000<((1L+(3L*A_i[(1L+A_rownnz[i])]))+(-3L*A_i[A_rownnz[i]])))) private(jj) reduction(+: tempx)
	...
	else
	{
		...
		#pragma cetus private(i, j, jj, temp) 
		...
		for (i=0; i<num_rows; i ++ )
		{
			if (num_vectors==1)
			{
				...
				#pragma cetus parallel 
				#pragma omp parallel for if((10000<((1L+(3L*A_i[(1L+i)]))+(-3L*A_i[i])))) private(jj) reduction(+: temp)
			...
			{
				#pragma cetus private(j, jj, temp) 
...
int hypre_CSRMatrixMatvecT(double alpha, hypre_CSRMatrix * A, hypre_Vector * x, double beta, hypre_Vector * y)
{
	...
	*/
	...
	{
		fprintf(stderr, "hypre_assert failed: %s\n", "num_vectors == hypre_VectorNumVectors(y)");
		hypre_error_handler("csr_matvec.c", 286, 1);
	}
	;
	if (num_rows!=x_size)
	{
	...
	*/
	num_threads=1;
	#pragma cetus private(i, j, jj, jv) 
	...
	*/
	...
	-----------------------------------------------------------------
	 y += A*x
	    *-----------------------------------------------------------------
	*/
	#pragma cetus private(i, jj, temp) 
	#pragma loop name hypre_CSRMatrixMatvec_FF#3 
	#pragma cetus parallel 
	#pragma omp parallel for private(i, jj, temp)
	for (i=0; i<num_rows; i ++ )
	{
		if (CF_marker_x[i]==fpt)
		{
