...
int hypre_BoomerAMGSeqRelax(hypre_CSRMatrix * A, hypre_Vector * f, hypre_Vector * u)
{
	...
	*/
	if (1)
	{
		...
		#pragma cetus private(i, ii, j, jj, ne, ns, res, rest, size) 
		...
		for (j=0; j<num_threads; j ++ )
		{
			...
			#pragma cetus private(i, ii, jj, res) 
			...
			for (i=ns; i<ne; i ++ )
			{
				...
				*/
				if (A_diag_data[A_diag_i[i]]!=0.0)
				{
					...
					#pragma cetus parallel 
					#pragma omp parallel for if((10000<((-5L+(6L*A_diag_i[(1L+i)]))+(-6L*A_diag_i[i])))) private(ii, jj) reduction(+: res)
		...
		for (i=0; i<n; i ++ )
		{
			...
			*/
			if (A_diag_data[A_diag_i[i]]!=0.0)
			{
				...
				#pragma cetus reduction(+: res) 
				#pragma cetus parallel 
				#pragma omp parallel for if((10000<((-3L+(4L*A_diag_i[(1L+i)]))+(-4L*A_diag_i[i])))) private(ii, jj) reduction(+: res)
				for (jj=(A_diag_i[i]+1); jj<A_diag_i[i+1]; jj ++ )
				{
					ii=A_diag_j[jj];
					res-=(A_diag_data[jj]*u_data[ii]);
