...
static void kernel_gramschmidt(int m, int n, double A[(2000+0)][(2600+0)], double R[(2600+0)][(2600+0)], double Q[(2000+0)][(2600+0)])
{
	...
	#pragma cetus private(i, j, k, nrm) 
	...
	for (k=0; k<n; k ++ )
	{
		...
		#pragma cetus parallel 
		#pragma omp parallel for if((10000<(((((-4L+(-5L*k))+(-6L*m))+(5L*n))+((-6L*k)*m))+((6L*m)*n)))) private(i, j) firstprivate(R) lastprivate(R)
		for (j=(k+1); j<n; j ++ )
		{
			R[k][j]=0.0;
			#pragma cetus private(i) 
			#pragma loop name kernel_gramschmidt#0#2#0 
			/* #pragma cetus reduction(+: R[k][j])  */
			for (i=0; i<m; i ++ )
			{
				R[k][j]+=(Q[i][k]*A[i][j]);
			}
